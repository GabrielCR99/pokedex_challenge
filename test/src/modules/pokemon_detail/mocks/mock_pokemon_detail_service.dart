...
final class MockPokemonDetailService extends Mock
  ...
  void mockFetchPokemonSpeciesFailure() =>
      when(() => fetchPokemonSpecies(any()))
          .thenThrow(const Failure(message: 'Failure'));
  void mockFetchNextPokemonDetailSuccess() =>
      when(() => fetchNextOrPreviousPokemonDetail(offset: any(named: 'offset')))
          .thenAnswer((_) async => pokemonDetailList[1]);
  void mockFetchNextPokemonDetailFailure() =>
      when(() => fetchNextOrPreviousPokemonDetail(offset: any(named: 'offset')))
          .thenThrow(const Failure(message: 'Failure'));
  void mockFetchPreviousPokemonDetailSuccess() =>
      when(() => fetchNextOrPreviousPokemonDetail(offset: any(named: 'offset')))
          .thenAnswer((_) async => pokemonDetailList.first);
